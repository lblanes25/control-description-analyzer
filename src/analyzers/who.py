...
def _process_detector_results(person_matches, system_matches, person_detector, system_detector, text):
    """Process results from new detector classes"""
    all_candidates = []
    
    # Process person matches
    for match in person_matches:
        confidence = person_detector.calculate_confidence(text, [match])
        all_candidates.append({
            "text": match["text"],
            "type": "person",
            "confidence": confidence,
            "positions": match.get("positions", []),
            "match_type": "detector_person"
        })
    
    # Process system matches
    for match in system_matches:
        confidence = system_detector.calculate_confidence(text, [match])
        all_candidates.append({
            "text": match["text"],
            "type": "system", 
            "confidence": confidence,
            "positions": match.get("positions", []),
            "match_type": "detector_system"
        })
    ...
    if not all_candidates:
        return {
            ...
            "secondary": [],
            "confidence": 0,
            "message": "No WHO detected using enhanced detectors"
        }
    
    # Sort by confidence and return top result
    all_candidates.sort(key=lambda x: x["confidence"], reverse=True)
    primary = all_candidates[0]
    secondary = [c["text"] for c in all_candidates[1:3]]
    
    return {
        "primary": primary["text"],
        "secondary": secondary,
        "confidence": primary["confidence"],
        "message": f"Enhanced detection found {primary['type']}: {primary['text']}"
    }