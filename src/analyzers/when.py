...
class TimingPatterns:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.MONTHLY = re.compile(patterns.get('monthly', r'\b(monthly|each\s+month|every\s+month|on\s+a\s+monthly\s+basis|month\b|monthly\s+basis)\b'), re.IGNORECASE)
        self.QUARTERLY = re.compile(patterns.get('quarterly', r'\b(quarterly|each\s+quarter|every\s+quarter|on\s+a\s+quarterly\s+basis|quarter\b|once\s+per\s+quarter|each\s+fiscal\s+quarter|every\s+three\s+months)\b'), re.IGNORECASE)
        self.ANNUALLY = re.compile(patterns.get('annually', r'\b(annually|yearly|each\s+year|every\s+year|annual\b|on\s+an\s+annual\s+basis|year\b)\b'), re.IGNORECASE)
        ...
        self.PERIOD_END = re.compile(patterns.get('period_end', r'\b(at|during|before|after)\s+(the\s+)?(fiscal|calendar)?\s*(year|quarter|month)[\s-]end(\s+close)?\b'), re.IGNORECASE)
        ...
        self.VAGUE_TERMS = re.compile(patterns.get('vague_terms', r'\b(as\s+needed|when\s+needed|if\s+needed|as\s+appropriate|when\s+appropriate|if\s+appropriate|as\s+required|when\s+required|if\s+required|periodically|occasionally|from\s+time\s+to\s+time|regularly|may\s+vary|may\s+change|may\s+differ|may\s+be|may\s+not|on\s+demand|as\s+and\s+when|upon\s+request|when\s+requested|non[\s-]scheduled)\b'), re.IGNORECASE)
